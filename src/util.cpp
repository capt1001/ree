...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Ree
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Ree
    // Mac: ~/Library/Application Support/Ree
    // Unix: ~/.Ree
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Ree";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "Ree.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "Reed.pid"));
